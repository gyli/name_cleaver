...
class PersonName(Name):
    ...
    def new_from_tokens(self, *args, **kwargs):
        ...
        if len(args):
            if self.is_an_honorific(args[-1]):
                ...
                # Only part of titles are abbreviation
                abb_title_re = 'Capt|Hon|st|Prof'
                if re.match(r'^([dm][rs]s?|m(iss|aster|aid|adam|x)|%s),?$' % abb_title_re, self.honorific, re.I) and not self.honorific[-1] == '.':
                    self.honorific += '.'
    ...
    def is_a_suffix(self, name_part):
        return re.match(r'^%s$' % SUFFIX_RE, name_part, re.I)

    def is_an_honorific(self, name_part):
        # return re.match(r'^\s*[dm][rs]s?[.,]?\s*$', name_part, re.IGNORECASE)
        return re.match(r'^\s*([dm][rs]s?|m(iss|aster|aid|adam|x)|%s)[.,]?\s*$' % TITLE_RE, name_part, re.I)
...
class FamilyPersonName(FamilyMetadata, PersonName):
    def __init__(self):
        self.isfamily = False


class FamilyMatesNames(FamilyMetadata):
