...
from names import SUFFIX_RE, DEGREE_RE, PersonName, PoliticianName, RunningMatesNames, \
    OrganizationName, FamilyPersonName, FamilyMatesNames
...
class IndividualNameCleaver(BaseNameCleaver):
    ...
    def separate_affixes(self, name):
        ...
        TITLE_RE = 'father|judge|Rabbi|S(ain)?t|Priest|Presbyter|Bishop|Friar|Sister|Prelate|Brother|Venerable|Capt(ain)?|Hon(ourable)?|Major|Private|Lieutenant|General|Sergeant|Admiral|Corporal|Commodore|Colonel|Representative|Senator|President|Mayor|(Lieutenant\s)?Governor|Prefect|Premier|Ambassador|Prof(essor)?|Dean|Minister'
        # name, honorific = self.extract_matching_portion(r'\b(?P<honorific>[dm][rs]s?[,.]?)(?=(\b|\s))+', name)
        name, honorific = self.extract_matching_portion(r'\b(?P<honorific>([dm][rs]s?|m(iss|aster|aid|adam|x)|%s)[,.]?)(?=(\b|\s))+' % TITLE_RE, name)
    ...
    def extract_matching_portion(self, pattern, name):
        m = re.finditer(pattern, name, flags=re.I)
...
class FamilyNameCleaver(IndividualNameCleaver):
    object_class = FamilyPersonName

    def __init__(self, string):
        super(FamilyNameCleaver, self).__init__(string)
    ...
    def parse(self, safe=False):
        ...
        if ' ' not in self.name:
            ...
            try:
                self.name = self.convert_familyname_to_obj(self.name)  # important for "last, first", and also running mates
            except Exception, e:
                return self.cannot_parse(safe, e)
            finally:
                if (isinstance(self.name, self.object_class) and self.name.last) or isinstance(self.name, FamilyMatesNames):
                    return self.name.case_name_parts()
                else:
                    return self.cannot_parse(safe)

    def convert_familyname_to_obj(self, name):
        # 'Mr. and Mrs. ...'
        family_title_re = r'^(M|D)r\.\s(and|&)\sMr?s\.\s'
        family_first_re = r'^.+\s(and|&)\s(.+\s)+'
        family_double_name_re = r'^.+\s.+.*(and|&)\s.+\s'
        family_get_1_re = r'\s(and|&).*$'
        family_get_2_re = r'^.+\s(and|&)\s'
        if re.match(family_title_re, name, flags=re.I):
            title1 = re.sub(r'^((M|D)r\.).*', '\\1', name)
            title2 = re.sub(r'^(M|D)r\.\s(and|&)\s(Mr?s\.)\s.*', '\\3', name)
            # Remove the title temporarily, then run this function again basically, but add title later
            name = re.sub(family_title_re, '', name)
            # Mr. and Mrs. First_1 and First_2 Last
            if re.match(family_first_re, name, flags=re.I):
                mate1_first = re.sub(family_get_1_re, '', name, flags=re.I)
                mate2 = title2 + ' ' + re.sub(family_get_2_re, '', name, flags=re.I)
                d = IndividualNameCleaver(mate2).parse()
                mate1 = title1 + ' ' + mate1_first + ' ' + d.last
                return self.convert_family_mates_names_to_obj(mate1, mate2)
            # Mr. and Mrs. First_1 Last_1 and First_2 Last_2
            elif re.match(family_double_name_re, name, flags=re.I):
                mate1 = title1 + ' ' + re.sub(family_get_1_re, '', name, flags=re.I)
                mate2 = title2 + ' ' + re.sub(family_get_2_re, '', name, flags=re.I)
                return self.convert_family_mates_names_to_obj(mate1, mate2)
            # Mr. and Mrs. First Last
            else:
                return self.convert_family_mates_names_to_obj(name, name)

        # First_1 and First_2 Last
        elif re.match(family_first_re, name, flags=re.I):
            mate1_first = re.sub(family_get_1_re, '', name, flags=re.I)
            mate2 = re.sub(family_get_2_re, '', name, flags=re.I)
            d = IndividualNameCleaver(mate2).parse()
            mate1 = mate1_first + ' ' + d.last
            return self.convert_family_mates_names_to_obj(mate1, mate2)

        # First_1 Last_1 and First_2 Last_2
        elif re.match(family_double_name_re, name, flags=re.I):
            mate1 = re.sub(family_get_1_re, '', name, flags=re.I)
            mate2 = re.sub(family_get_2_re, '', name, flags=re.I)
            return self.convert_family_mates_names_to_obj(mate1, mate2)

        else:
            # return self.convert_regular_name_to_obj(name)
            name = self.pre_process(name)
            mainname, honorific, suffix, nick = self.separate_affixes(name)
            # if honorific and not honorific.endswith('.'):
            #     honorific += '.'
            mainname = self.reverse_last_first(mainname)
            return self.convert_name_to_obj(mainname, nick, honorific, suffix)

    # convert_regular_name_to_obj is usless since we still need to use IndivNameCleaver method to clean both names
    # def convert_regular_name_to_obj(self, name):
    #     name = self.reverse_last_first(name)
    #     return self.get_object_class().new_from_tokens(*[x for x in re.split('\s+', name) if x])

    def convert_family_mates_names_to_obj(self, mate1, mate2):
        return FamilyMatesNames(*[self.convert_familyname_to_obj(mate1), self.convert_familyname_to_obj(mate2)])
